...
function baseData(): FxResponse {
  return {
    base:'EGP', currency:'EGP', rate:1, usdtRate:null, usdtUsdPrice:null,
    source:'base', cachedAt:new Date().toISOString(), status:'base',
  };
}

function readCached(currency: CurrencyCode): FxResponse | null {
  const entry = fxCache.get(currency);
  if (entry && Date.now() - entry.ts < FX_CACHE_TTL) return { ...entry.data, status:'stale' };
  try {
...
export function useFx(currency: CurrencyCode) {
  ...
  useEffect(() => {
    ...
    if (currency === 'EGP') {
      setData(baseData()); setLoading(false); setError(false); return;
    }
    ...
    fetch(`${FX_URL}/api/fx?currency=${currency}`, { signal:controller.signal, cache:'no-store' })
      .then(async (response) => {
      ...
      .catch((err) => {
        ...
        if (fallback) setData(fallback); else setError(true);
      })
